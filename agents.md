...
## Issue Tracking with wedow/ticket (`tk`)

This project uses **wedow/ticket** via the `tk` CLI. Tickets are stored in `.tickets/` and should be committed with related work.
...
tk create "Title" -d "Details" -t task -p 2
tk start <id>                    # mark in_progress
tk close <id>                    # mark closed
tk add-note <id> "Note"          # append progress/context
```

### Important Rules

- Use `tk` for all repo issue tracking.
- Do not create ad-hoc TODO files or duplicate tracking systems.
- Create tickets for discovered follow-up work.
- Commit `.tickets/` changes with the code/docs changes they describe.
